...
import { Calendar, Flame, GraduationCap, Inbox } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      {courses.length === 0 ? (
        <motion.div
          variants={itemVariants}
          className="col-span-1 md:col-span-2 lg:col-span-3 flex flex-col items-center justify-center border border-dashed border-zinc-800 rounded-2xl bg-zinc-950/40 p-12 text-zinc-400 min-h-[250px]"
        >
          <Inbox className="h-8 w-8 text-zinc-600 mb-3 animate-pulse" />
          <p className="text-base font-semibold text-zinc-300">No active courses mapped</p>
          <p className="text-xs text-zinc-500 max-w-xs text-center mt-1">
            Your connection is secure. Please add mock rows to your Supabase <code className="text-violet-400 font-mono">courses</code> table to run telemetry.
          </p>
        </motion.div>
      ) : (
        courses.map((course) => (
